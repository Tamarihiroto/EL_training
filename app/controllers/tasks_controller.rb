class TasksController < ApplicationController
  ...
  def index
    @tasks = Task.all.recent().split_tasks_on_a_page(params[:page])
  end
  ...
  def sort_by_order
    ...
    @tasks = Task.sorted(column: column, direction: direction).split_tasks_on_a_page(params[:page])
    render :index
  end
  ...
  def search
    ...
    if is_search_params_nil?(search_params)
      ...
      @tasks = Task.search(search_params).split_tasks_on_a_page(params[:page])
    end
