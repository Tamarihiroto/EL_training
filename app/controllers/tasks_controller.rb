class TasksController < ApplicationController
  ...
  def index
    @tasks = Task.all.recent().page(params[:page]).per(20)
  end
  ...
  def sort_by_order
    ...
    @tasks = Task.sorted(column: column, direction: direction).page(params[:page]).per(20)
    render :index
  end
  ...
  def search
    ...
    if is_search_params_nil?(search_params)
      ...
      @tasks = Task.search(search_params).page(params[:page]).per(20)
    end
